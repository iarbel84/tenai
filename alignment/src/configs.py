...
class DataArguments:
    ...
    dataset_name: Optional[str] = field(
        default=None,
        metadata={"help": ("Dataset path for training.")},
    )
    dataset_splits: Optional[Dict[str, str]] = field(
        default_factory=lambda: {"train": "train", "validation": "validation"},
        metadata={"help": ("Dict of train / validation splits to use in the dataset.")},
    )
    prompt_template: Optional[str] = field(
        default=None,
        metadata={"help": ("Data formatting template, e.g. 'Instruction: {INSTRUCTION}\n###Input: {INPUT}'.")},
    )
    prompt_columns: Optional[Dict[str, str]] = field(
        default=None,
        metadata={"help": ("Columns from the dataset to use in the prompt template. Input a dict where keys are the prompt template arguments and values are the corresponding columns.")},
    )
    response_template: Optional[str] = field(
        default=None,
        metadata={"help": ("The template form that indicates the start of the response, typically something like '### Response:\n{RESPONSE}'.")},
    )
    response_token_ids: Optional[List[int]] = field(
        default=None,
        metadata={"help": ("Token IDs of the response template. It's recommended to provide response_token_ids directly, as some tokenizers (e.g. Llama-2) tokenize sequences differently depending whether they have context or not.")},
    )
    response_column: Optional[Dict[str, str]] = field(
        default=None,
        metadata={"help": ("Column from the dataset to use in the response template. Input a dict where the key is the response template argument and the value is the corresponding column.")},
    )
